...
async function fetching(folder) {
...
async function playingSongs() {
  let songList = [];
  let as =await fetching('trending');
  for (a of as) {
    if (a.innerText.endsWith(".mp3")) {
      songList.push(a.innerText);
    }
  }
  for (song of songList) {
    console.log("trending/"+song.split("-")[0].slice(0,-1)+".webp")
...
async function playthis(song,folder) {
  ...
  for (a of as) {
    if (a.innerText.endsWith(".mp3")) {
      currentPlaylist.push(a.innerText);
    }
  }
...
async function listingPlaylists(){
  ...
  for(folder of myFolders){
    document.getElementById('herePlaylists').innerHTML += `<div onclick="listSongs('${folder}')" class="song flex">
                                                        <div class="thumbnail"><img src="http://127.0.0.1:3000/playlists/${folder}/cover.jpg" alt="cover"></div>
                                                        <div class="forhover">
                                                          <div class="playbtn flex"><i class="fa-solid fa-play"></i></div>
                                                        </div>
...
async function listSongs(folder) {
  ...
  let a = await fetch(`http://127.0.0.1:3000/playlists/${folder}`);
  let response = await a.text();
  let div = document.createElement("div");
  div.innerHTML = response;
  let as=div.getElementsByTagName('a')
...
async function listArtists() {
  ...
  let as=div.getElementsByTagName('a')
  for(a of as){
    if(a.innerText.endsWith('.jpg'))
    artists.push(a.innerText);
  }
  for(art of artists){
